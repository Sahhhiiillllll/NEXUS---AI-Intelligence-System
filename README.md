...
| `NEXUS_DEMO_MODE=true` | No | Offline fallback responses |
...
```bash
git clone https://github.com/Sahhhiiillllll/NEXUS---AI-Intelligence-System-LLM.git
cd NEXUS---AI-Intelligence-System-LLM
```
...
6. NEXUS routes your query to the right tool and responds via chat + TTS.

### Example queries

| Query | Tool |
|-------|------|
| "Hello NEXUS" | LLM greeting |
...
2. Add environment variables in the Vercel dashboard:
   ...
   | `NEXUS_CITY` | Default city, e.g. `New York` |
...
| `NEXUS_HOST` | WebSocket bind host (local) | `localhost` | No |
| `NEXUS_PORT` | WebSocket port (local) | `8765` | No |
| `NEXUS_DEBUG` | Verbose logging | `false` | No |
| `NEXUS_DEMO_MODE` | Offline fallback mode | `false` | No |
| `NEXUS_CITY` | Default weather city | `New York` | No |
| `NEXUS_HTTP_TIMEOUT` | HTTP timeout (seconds) | `15` | No |
| `NEXUS_MAX_RETRIES` | API retry count | `3` | No |
| `NEXUS_FAILURE_THRESHOLD` | Circuit breaker threshold | `5` | No |
| `NEXUS_ENABLE_METRICS` | Prometheus (local) | `true` | No |
...
```
NEXUS---AI-Intelligence-System-LLM/
├── api/
